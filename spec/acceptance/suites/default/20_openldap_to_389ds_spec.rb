...
describe 'OpenLDAP to 389DS convert and import scripts' do
  ...
  hosts.each do |host|
    context "Executing converter script on #{host}" do
      ...
      it 'converts a valid input file using default options' do
      ...
      it 'converts a valid input file using specified options' do
      ...
      it 'fails when the input file does not exist' do
        cmd = [
          '/usr/bin/ruby',
          converter,
      ...
      it 'fails when the input file does not contain valid LDIF data' do
        cmd = [
          '/usr/bin/ruby',
          converter,
      ...
      it 'fails when base DN is not specified and cannot be determined from input LDIF' do
        cmd = [
          '/usr/bin/ruby',
          converter,
      ...
      it 'fails when the output file cannot be created' do
        cmd = [
          '/usr/bin/ruby',
          converter,
          "-i #{openldap_ldif}",
      ...
      it 'fails when the input file has no users or groups for base DN' do
        cmd = [
          '/usr/bin/ruby',
          converter,
          "-i #{openldap_ldif}",
  ...
  context "Executing importer script on #{ldap_server}" do
    ...
    shared_examples_for 'a LDAP import validator' do
      it 'creates a searchable LDAP tree' do
    ...
    context 'import into a base accounts instance using specified input file' do
      it_behaves_like 'an accounts 389DS instance installer'

      it 'executes without error using specified LDIF file' do
        on(ldap_server, "#{importer} #{ds389_ldif}")
      end

      it_behaves_like 'a LDAP import validator'
    end

    context 'import into a base accounts instance using default input file' do
      it_behaves_like 'an accounts 389DS instance installer'

      it 'executes without error using default LDIF file' do
    ...
    context 'import failures' do
      ...
      it 'fails if there are no People or Groups in the LDIF to import' do
      ...
      it 'fails to import into an accounts instance with non-empty users or administrators groups' do
      ...
      it 'fails if the accounts 389ds instance does not exist' do
        apply_manifest_on(ldap_server, remove_ldap_server_manifest, catch_failures: true)
        on(ldap_server, "#{importer} #{ds389_ldif}", acceptable_exit_codes: [1])
      end

      it 'fails if it cannot find the dsidm command' do
        # dsidm is in /usr/sbin
        on(ldap_server, "PATH=/usr/bin #{importer} #{ds389_ldif}", acceptable_exit_codes: [1])
      end
    end
  end
end
